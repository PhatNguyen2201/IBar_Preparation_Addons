...
def generate_license_key(hardware_id: str) -> str:
    # Keep behavior identical to addon: sha512((hardware_id * 2))
    return create_hash(hardware_id * 2)


def build_parser() -> argparse.ArgumentParser:
    default_hwid = Path.home() / "Desktop" / "IbarPrep.hwid"
    default_output = Path.home() / "addon_ibar.key"

    parser = argparse.ArgumentParser(
        description="Tao addon_ibar.key tu file IbarPrep.hwid"
    )
    parser.add_argument(
        "--hwid",
        type=Path,
        default=default_hwid,
        help=f"Duong dan den file HWID (mac dinh: {default_hwid})",
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=default_output,
        help=f"Duong dan file key output (mac dinh: {default_output})",
    )
    parser.add_argument(
        "--print-only",
        action="store_true",
        help="Chi in key ra man hinh, khong ghi file",
    )
    return parser
